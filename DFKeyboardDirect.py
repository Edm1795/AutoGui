...
class TaskSet:
    ...
    def moveMouseNEW(self, time, shift, duration=1):
        ...
        sBase = 378 # 405  # base value for shift which is the top shift showing on screen (vertical axis)
    ...
    def newDrag(self, duration=1, button='l'):
        ...
        startTime=int(input('Type start time:'))
        shift=int(input('Type shift number'))

        self.moveMouseNEW(startTime, shift)

        shift = shift - 1  # decrement by one to get correct results
        shiftDiff = 52  # pixel distance between shifts (adjust this if mouse is not accurante
        tBase = 524  # base value for time which is 8 am (horizontal axis)
        sBase = 378  # base value for shift which is the top shift showing on screen (vertical axis)

        timeUnit = 30  # number of pixels for a time unit (set at 30 mins;ie 38px = 30 mins of time)

        timeSteps = self.timeSteps(startTime+1)  # number of time steps from base (8am) to desired time
